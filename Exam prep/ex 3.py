...
def take_damage(heroes_dict, hero_name, damage, attacker):
    ...
    healths_points_left = current_health_points - damage
    if healths_points_left <= 0:
        del heroes_dict[hero_name]
        print(f"{hero_name} has been killed by {attacker}!")
    else:
        heroes_dict[hero_name]['HP'] = healths_points_left
...
for _ in range(n):
    ...
    heroes[name] = {"HP": health_points, "MP": mana_points}
